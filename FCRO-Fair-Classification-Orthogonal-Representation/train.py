...
def main():
    ...
    parser.add_argument("--batch_size", help="batch size", type=int, default=32)#default=128)
    parser.add_argument(
        "--target_label",
    ...
    parser.add_argument(
        ...
        default="./metadata/METADATA_reduced.xlsx",
    )
    ...
    model_a = BetaVAE(latent_size=128, beta=100).cuda()
    ...
    if args.test:
        ...
        model_t.load_state_dict(
            torch.load(os.path.join(args.pretrained_path, f"fold_{args.fold}", f"model_target_{args.epoch}.pth"))[
                "state_dict"
            ]
        )
    ...
    if args.from_sketch:
        ...
        checkpoint_file = os.path.join(
            ...
            "../Unsupervised/model_weights_beta100_epoch10.pth"
        )
